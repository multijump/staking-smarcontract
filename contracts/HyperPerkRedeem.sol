...
contract SecondaryTokenRedeem is Ownable {
    NFT public NFTContract;
    SecondaryTokenNonTradable public SecondaryToken;
    mapping(uint256 => uint256) public itemCosts;

    event ItemAdded(uint256 item, uint256 points);
    event Redeemed(address indexed user, uint256 amount);

    constructor(NFT _NFTContractAddress, SecondaryTokenNonTradable _secondaryTokenAddress) public {
        NFTContract = _NFTContractAddress;
        SecondaryToken = _secondaryTokenAddress;
    }

    function addItem(uint256 itemId, uint256 amount) public onlyOwner {
        require( amount >= 0, 'price amount not correct');
        itemCosts[itemId] = amount;
        emit ItemAdded(itemId, amount);
    }

    function redeem(uint256 item) public {
        require(itemCosts[item] != 0, "Item not found");
        require(SecondaryToken.balanceOf(msg.sender) >= itemCosts[item], "Not enough points to redeem for item");
        require(NFTContract.totalSupply(item) < NFTContract.maxSupply(item), "Max items minted");

        SecondaryToken.burn(msg.sender, itemCosts[item]);
        NFTContract.mint(msg.sender, item, 1, "");
        emit Redeemed(msg.sender, itemCosts[item]);
    }
}